const dataPost = [
  {
    id:0,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:1,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 36,
    totalHeart: 50,
    type: "music",
  },
  {
    id:2,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 11,
    type: "music",
  },
  {
    id:3,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:4,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:5,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "media",
  },
  {
    id:6,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "social",
  },
  {
    id:7,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:8,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:9,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:10,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:11,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:12,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:13,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 36,
    totalHeart: 50,
    type: "video",
  },
  {
    id:14,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:15,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:16,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:17,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:18,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:19,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:20,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:21,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:22,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:23,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:24,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:25,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 36,
    totalHeart: 50,
    type: "video",
  },
  {
    id:26,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:27,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:28,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "video",
  },
  {
    id:29,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:30,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:31,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:32,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 22,
    type: "music",
  },
  {
    id:33,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "music",
  },
  {
    id:34,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:35,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:36,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:37,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:38,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:39,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:40,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:41,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:42,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:43,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "music",
  },
  {
    id:44,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:45,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:46,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:47,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:48,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:49,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:50,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:51,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:52,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:53,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:54,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:55,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:56,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:57,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:58,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 5,
    type: "news",
  },
  {
    id:69,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 44,
    type: "news",
  },
  {
    id:60,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 77,
    type: "news",
  },
  {
    id:61,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 99,
    type: "news",
  },
  {
    id:62,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 50,
    type: "news",
  },
  {
    id:63,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 41,
    type: "news",
  },
  {
    id:64,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 20,
    type: "video",
  },
  {
    id:65,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:66,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:67,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:68,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:69,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:70,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:71,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:72,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:73,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:74,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:75,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:76,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:77,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:78,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:79,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:80,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:81,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:82,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:83,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:84,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:85,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:86,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:87,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:88,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:89,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:90,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:91,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:92,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:93,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:94,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:95,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:96,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:97,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:98,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:99,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  },
  {
    id:100,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sitfissimos obcaecati molestias nisidoloribus vero, tempore minima adipisci",
    eventTime: 1614767700000,
    imageUrl: "./images/content-img/12795f600c5c4ed2974b5d547663d1a6.jpg",
    link: "",
    favorited: true,
    totalComment: 0,
    totalHeart: 35,
    type: "news",
  }
];

//  assignment1: load data, each load show page contains 20 items.
function render(item) {
  let html = `    
  <div class="item">
  <div class="item--content">
    <div class="content__avatar">
      <img
        src=${item.imageUrl}
        alt=""
      />
    </div>
    <div class="content__description">
      <h3>messi</h3>
      <p>${item.description}</p>
      <span style="font-size: 2rem">${item.type}</span>
    </div>
    <div class="content__reaction">
      <div class="content__reaction--time">
        <span>20 hours</span>
      </div>
      <div class="content__reaction--interative">
        <span class="heart" data-id="${item.id}">
          <i class="fas fa-heart"></i>
          <span class="counter__heart">${item.totalHeart}</span>
        </span>
        <span style="margin-left:0.5rem">
          <i class="fas fa-comment"></i>
          <span class="counter__comment">${item.totalComment}</span>
        </span>
      </div>
    </div>
  </div>
</div>`;
  return html;
}
const masonry__wrapper = document.querySelector(".masonryLayout__wrapper");

function showPost(items) {
  let html = " ";
  let id = 0;
  let indexPost = [];
  while (indexPost.length < 20) {
    id = Math.floor(Math.random() * items.length );
    if (!indexPost.includes(id)) indexPost.push(id);
  }
  indexPost.forEach((index) => (html += render(items[index])));
  masonry__wrapper.insertAdjacentHTML("beforeend", html);
  changeStatusIconReaction()
  
}

function load() {
  dataPost.forEach(item =>{
    if(item.type == "music"){
      item.imageUrl = "./images/content-img/29f58d6a-lionel-messi-.jpeg"
    }else if(item.type == "news"){
      item.imageUrl = "./images/content-img/d58d9070-879a-11eb-949a-6d37d6280f43_medium.jpg"
    }
  })

  showPost(dataPost);
  showFilter();
  // changeStatusIconReaction()
}
load()
scrollPage(dataPost);


var countPage = 1;
function scrollPage(data) {
  document.onscroll = () => {
    const { scrollHeight, scrollTop, clientHeight } = document.documentElement;
    if ( scrollTop >= scrollHeight - clientHeight - 1   && countPage < 5) {
      
      showPost(data)
      // changeStatusIconReaction()
      countPage++;
      console.log(countPage);
    }
  };
}


// Assignment 2: filter by name and show item

function getByType(type) {
  let listnew = [];
  dataPost.forEach((item, index) => {
    if (item.type.toLowerCase() == type) {
      listnew.push(item);
    }
  });
  showPost(listnew);
  // console.log(listnew);
}

function showFilter() {
  const navigationbtn = document.querySelectorAll(".navigation__menu li a");
  for (let i = 0; i < navigationbtn.length; i++) {
    navigationbtn[i].addEventListener("click", (e) => {
      e.preventDefault();
      listnew = [];
      masonry__wrapper.textContent = "";
      getByType(e.target.innerText.toLowerCase());
      // changeStatusIconReaction()
    });
  }
};
///assignment 3: update status of reacting activities
function changeStatusIconReaction() {
  // load();
  const iconHeartCounter = Array.from( document.querySelectorAll(".counter__heart"));
  const iconHearts = Array.from(document.querySelectorAll(".heart > i"));
  console.log(iconHearts)
  iconHearts.forEach((item,index)=>{
    item.addEventListener("click",(e)=>{
      item.classList.toggle('active')
      let parentid = e.target.parentNode.dataset.id;
      
      console.log(item)
      console.log(e.target.parentNode.dataset.id)
    })
  })
}


// function test(){
//   load();
//   const arritems = [...document.querySelectorAll('.item')];
//   arritems.forEach((item,index)=>{
//     item.addEventListener("click",(e)=>{
//       console.log("đây là phần tử thứ" + item[])
//     })
//   })
// }
// test()

